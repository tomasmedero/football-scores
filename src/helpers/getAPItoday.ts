...
export const getAPItoday = async (): Promise<LeagueInfo[]> => {
  const todayDate = new Date()
  const toDateString = todayDate.toISOString().split('T')[0]

  const url = `https://api-football-v1.p.rapidapi.com/v3/fixtures?date=${toDateString}&status=NS`
  ...
  data.response.forEach((match: any) => {
    ...
    const existingLeague = matchToday.find(
